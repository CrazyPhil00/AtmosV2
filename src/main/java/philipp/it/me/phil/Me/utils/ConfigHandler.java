...
public class ConfigHandler {
    ...
    private static String file = "config/Atmos.cfg";
    ...
    public static int[] getIntList(String category, String key) {
        config = new Configuration(new File(file));
        try {
            config.load();
            if (config.getCategory(category).containsKey(key)) {
                return config.get(category, key, 0).getIntList();
            }
        } catch (Exception e) {
            System.out.println("Cannot load configuration file!");
        } finally {
    ...
    public static void setComment(String category, String comment) {
        config = new Configuration(new File(file));
        try {
            config.load();
            if (!config.hasCategory(category)) return;
            config.getCategory(category).setComment(comment);
        } catch (Exception e) {
            System.out.println("Cannot load configuration file!");
        } finally {
            config.save();
